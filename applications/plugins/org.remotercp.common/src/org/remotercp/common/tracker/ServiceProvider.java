...
public final class ServiceProvider {
	...
	public static synchronized <S> S getService(Class<S> clazz) {
		assert hasService(clazz) : "Precondition violated: hasService(" + clazz
				+ ") == true";
	 ...
	 */
	static synchronized final <S> void addService(Class<?> clazz, S service) {
		if (services.containsKey(clazz)) {
			throw new IllegalArgumentException("an implementation of '"
					+ clazz.getName() + "' was already added");
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	static synchronized final <S> void removeService(Class<S> clazz) {
	 ...
	 *            - the listener class
	 */
	public static synchronized final <S> void addServiceListener(
			Class<S> clazz, ServiceListener<S> listener) {
	 ...
	 *            - the listener class
	 */
	public static synchronized final <S> void removeServiceListener(
			Class<S> clazz, ServiceListener<S> listener) {
		if (listeners.get(clazz) != null) {
			listeners.get(clazz).remove(listener);
		}

		// if there are no more listeners, we remove the service bridge to save
		// resources
	...
	private static synchronized <S> Object getServicePrivate(Class<S> clazz) {
		// check if the service bridge is already added for this class
		if (!bridges.containsKey(clazz)) {
			// not added. this means we have to add the service bridge so the
			// service becomes available to the ServiceProvider
			addServiceBridge(clazz);
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static synchronized final <S> void notifyListenersForServiceAdded(
			Class<S> clazz) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static synchronized final <S> void notifyListenerForServiceAdded(
			ServiceListener<S> listener, Class<S> clazz) {
		ServiceListener<S> casted = (ServiceListener<S>) listener;
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static synchronized final <S> void notifyListenersForServiceRemoved(
	 ...
	 */
	private static synchronized final <S> void addServiceBridge(Class<S> clazz) {
