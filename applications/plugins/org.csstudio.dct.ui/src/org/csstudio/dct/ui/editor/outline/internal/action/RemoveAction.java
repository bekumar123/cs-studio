...
import org.csstudio.dct.ui.editor.outline.internal.SelectInOutlineCommand;
...
public final class RemoveAction extends AbstractOutlineAction {
    ...
    private boolean confirm(String message) {
        MessageDialog d = new MessageDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
                "Confirmation", null, message, MessageDialog.WARNING, new String[] { "Ok", "Cancel" }, 0);
    ...
    private void collectElementsToDelete(IFolder folder, List<IFolder> folders, Set<IPrototype> prototypes,
            Set<IInstance> instances) {
    ...
    protected void afterSelectionChanged(List<IElement> selection, IAction action) {
        ...
        // @formatter:off
        boolean specialFolder = CompareUtil.containsInstancesFolder(selection)
                || CompareUtil.containsPrototypesFolder(selection) 
                || CompareUtil.containsLibraryFolder(selection);
                // @formatter:on
        ...
        // @formatter:off
        boolean enabled = (CompareUtil.containsOnly(IPrototype.class, selection)
                || CompareUtil.containsOnly(IInstance.class, selection)
                || CompareUtil.containsOnly(IFolder.class, selection) 
                || onlyRecords) 
                && (!specialFolder)
                && (!childOfLibraryFolder);
                // @formatter:on
