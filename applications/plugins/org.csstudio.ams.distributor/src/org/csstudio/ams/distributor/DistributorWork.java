...
public class DistributorWork extends Thread implements AmsConstants, MessageListener {
		...
		while (bStop == false) {
			try {
				...
				synchronized (synchronizer) {
					...
					if (iErr == ErrorState.STAT_OK.getStateNumber()) {
						final List<Integer> keyList = MessageChainDAO.selectKeyList(memoryCacheDb, MESSAGECHAIN_WORK);
	...
	private boolean initJmsInternal() {
		...
		String topicName = storeAct.getString(AmsPreferenceKey.P_JMS_AMS_TOPIC_SMS_CONNECTOR);
		...
		try {
			...
			// The topic T_AMS_DISTRIBUTE is now used by the connection that is
			// created in the
			// application class. The messages are processed by the method
			// onMessage()
			// success = amsReceiver
			...
			success = amsReceiver.createRedundantSubscriber("amsSubscriberReply", storeAct.getString(AmsPreferenceKey.P_JMS_AMS_TOPIC_REPLY),
					storeAct.getString(AmsPreferenceKey.P_JMS_AMS_TSUB_REPLY), durable);
	...
	private boolean initJmsExternal() {
		...
		try {
			...
			properties.put(Context.INITIAL_CONTEXT_FACTORY, storeAct.getString(AmsPreferenceKey.P_JMS_EXTERN_CONNECTION_FACTORY_CLASS));
			properties.put(Context.PROVIDER_URL, storeAct.getString(AmsPreferenceKey.P_JMS_EXTERN_SENDER_PROVIDER_URL));
			extContext = new InitialContext(properties);

			extFactory = (ConnectionFactory) extContext.lookup(storeAct.getString(AmsPreferenceKey.P_JMS_EXTERN_CONNECTION_FACTORY));
			...
			// TODO: ATTENTION!!!
			// We have to get the topic for re-insert the alarm message from a
			// different
			// preference entry! AmsPreferenceKey.P_JMS_EXT_TOPIC_ALARM is not
			// usable
			// because it could be contain one OR MORE alarm topics (ALARM,
			// SNL_LOG, ...).
			// In case of re-insertion of an alarm chain message, it will be
			// sent to ALL
			// topics!!!! This will cause more message chains.

			extPublisherAlarm = extSession
					.createProducer(extSession.createTopic(storeAct.getString(AmsPreferenceKey.P_JMS_EXT_TOPIC_ALARM_REINSERT)));
	...
	private void publishToConnectorSms(final String text, final String addr) throws JMSException {
	 ...
	 * @throws JMSException
	 */
	private void publishToConnectorJms(final String text, final String topic, final HashMap<String, String> map) throws JMSException {
		final MapMessage msg = amsSender.createMapMessage();
		...
		Log.log(Log.INFO, "DistributorWork.publishToConnectorJms(): Message sent via amsPublisherJms.send([text=\"" + text + "\", topic=\"" + topic
				+ "\"]);");
	}

	private void publishToConnectorMail(final String text, final String addr, final String username) throws JMSException {
		final MapMessage msg = amsSender.createMapMessage();
		msg.setString(MSGPROP_RECEIVERTEXT, text);
		msg.setString(MSGPROP_RECEIVERADDR, addr);
		Log.log(Log.INFO,
				"DistributorWork.publishToConnectorMail() -1- addr=" + msg.getString(MSGPROP_RECEIVERADDR) + ", username="
						+ msg.getString(MSGPROP_SUBJECT_USERNAME));
		msg.setString(MSGPROP_SUBJECT_USERNAME, username);
		Log.log(Log.INFO,
				"DistributorWork.publishToConnectorMail() -2- addr=" + msg.getString(MSGPROP_RECEIVERADDR) + ", username="
						+ msg.getString(MSGPROP_SUBJECT_USERNAME));
		amsSender.sendMessage("amsPublisherMail", msg);
	}

	private void publishToConnectorVoiceMail(final String text, final String addr, final int texttype) throws JMSException {
		publishToConnectorVoiceMail(text, addr, "", texttype);
	}

	private void publishToConnectorVoiceMail(final String text, final String addr, final String chainIdAndPos, final int texttype)
			throws JMSException {
		final MapMessage msg = amsSender.createMapMessage();
		msg.setString(MSGPROP_RECEIVERTEXT, text);
		msg.setString(MSGPROP_RECEIVERADDR, addr);
			...
			if (!(message instanceof MapMessage)) {
				...
				if (val != null && val.equals(MSGVALUE_TCMD_RELOAD_CFG_START)) {
					...
					if (iErr == ErrorState.STAT_FALSE.getStateNumber()) {
						Log.log(this, Log.WARN, "Could not distributeMessage, handle as O.K.");
	...
	private int distributeMessage(final MapMessage msg) throws Exception {
		...
		try {
			...
			final String description = "Message filtered by " + iFilterId + " - " + (filter == null ? "filter not there" : filter.getName()) + "."
			...
			final List<?> fActions = AggrFilterActionDAO.select(memoryCacheDb, iFilterId);
			...
			while (iter.hasNext()) {
				...
				if (fa.getFilterActionTypeRef() == FILTERACTIONTYPE_SMS_GR || fa.getFilterActionTypeRef() == FILTERACTIONTYPE_VM_GR
						|| fa.getFilterActionTypeRef() == FILTERACTIONTYPE_MAIL_GR) {

					// ADDED BY Markus Moeller, 2007-11-12
					// Blocking non-active groups
					final AggrUserGroupTObject userGroup = AggrUserGroupDAO.selectList(memoryCacheDb, fa.getReceiverRef());
					if (userGroup.getUsergroup().getIsActive() != 0) {
						if (iMessageId == -1) {
							iMessageId = MessageDAO.insert(localAppDb, msg, true);
							// Make sure that message IDs are identical in both
							// DBs
							MessageDAO.insertWithMessageId(memoryCacheDb, msg, iMessageId);
						}

						final MessageChainTObject messageChainObject = new MessageChainTObject(-1, iMessageId, filter.getFilterID(),
								fa.getFilterActionID(), -1, null, null, MESSAGECHAIN_WORK, null);
						...
						HistoryWriter.logHistoryGroupBlocked(localAppDb, msg, "Send to group with reply", fa.getFilterActionTypeRef(),
								userGroup.getUsergroup(), 0, 0, TopicDAO.select(memoryCacheDb, fa.getReceiverRef()));
					}
				} else {
					final int iErr = sendMessage(msg, filter, fa); // throws
					// Exception
					if (iErr == ErrorState.STAT_OK.getStateNumber() || iErr == ErrorState.STAT_FALSE.getStateNumber()) {
						if (iWorked == ErrorState.STAT_FALSE.getStateNumber()) {
							iWorked = iErr;
						}
					} else {
	...
	private String prepareMessageText(final MapMessage mapMsg, final FilterTObject filter, final FilterActionTObject fa,
																	...
																	// AMSException
	{
		...
		if (nextChain != null) // add MessageChain for Reply
		{
			...
			sbText.append(prepareMessageNumber(nextChain.getMessageChainID(), nextChain.getReceiverPos()));
		}

		return sbText.toString();
	}

	private String prepareMessageNumber(final int iChainID, final int iChainPos) throws Exception // INCLUDING
																									// -
																									// AMSException
	{
		int iRlen = ("" + iChainPos).length();
		if (iRlen > MSG_POS_LENGTH_FOR_MSGPROP) {
			throw new AMSException("MessageChain ReceiverPos=" + iChainPos + " has more chars than > MSG_POS_LENGTH_FOR_MSGPROP="
	...
	private int sendMessage(final MapMessage mapMsg, final FilterTObject filter, final FilterActionTObject fa) throws Exception // INCLUDING
																																// -
	// SQLException,
	// JMSException,
	// AMSException
	{
		...
		if (faTypeRef >= FILTERACTIONTYPE_TOPICDEST) {
			...
			throw new AMSException("Configuration is invalid. FilterActionType=" + faTypeRef);
		}
	}
	...
	private void sendMessageToDefaultTopic(final Message message) {
		...
		Log.log(this, Log.WARN, "method sendMessageToDefaultTopic(Message message) not implemented yet!");
	}

	private int sendMessageToConnector(final MapMessage mapMsg, final String text, final FilterActionTObject fa) throws Exception // INCLUDING
																																	// -
	// SQLException,
	// JMSException,
	// AMSException
	{
		...
		HistoryWriter.logHistorySend(localAppDb, mapMsg, text, fa.getFilterActionTypeRef(), user, null, -1, 0, 0, topic);
	...
	private int sendMessageToUserGroup(final MapMessage mapMsg, final String text, final FilterActionTObject fa) throws Exception // INCLUDING
																																	// -
	// SQLException,
	// JMSException,
	// AMSException
	{
		...
		if (userGroup.getUsergroup().getIsActive() == 0) {
			HistoryWriter.logHistoryGroupBlocked(localAppDb, mapMsg, text, fa.getFilterActionTypeRef(), userGroup.getUsergroup(), 0, 0,
		...
		while (iter.hasNext()) {
			final AggrUserGroupUserTObject aUser = (AggrUserGroupUserTObject) iter.next();
			final UserTObject user = aUser.getUser();

			if (aUser.getUserGroupUser().getActive() == 0) {
				Log.log(Log.WARN, "UserGroupRel not active: User " + user.getUserID() + " of Group " + aUser.getUserGroupUser().getUserGroupRef()
			...
			HistoryWriter.logHistorySend(localAppDb, mapMsg, text, fa.getFilterActionTypeRef(), aUser.getUser(), userGroup.getUsergroup(), -1, 0, 0,
					TopicDAO.select(memoryCacheDb, fa.getReceiverRef()));
	...
	private int responseMsg(final Message message) throws Exception {
		...
		try {
			if (!(message instanceof MapMessage)) {
				...
				if (iErr == ErrorState.STAT_FALSE.getStateNumber()) {
					Log.log(this, Log.WARN, "MapMessage not accepted => delete it");
	...
	// InterruptedException
	{
		...
		if (strChainIdAndPos != null) {
			...
			try {
				if (!strChainIdAndPos.equals("#")) {
					if (strChainIdAndPos.length() < MSG_POS_LENGTH_FOR_MSGPROP + 1) {
						throw new NumberFormatException("strChainIdAndPos.length() < " + (MSG_POS_LENGTH_FOR_MSGPROP + 1));
					}

					final int posInStr = strChainIdAndPos.length() - MSG_POS_LENGTH_FOR_MSGPROP;// chars
																								// used
																								// for
					// pos
					chainPos = Integer.parseInt(strChainIdAndPos.substring(posInStr));
					chainID = Integer.parseInt(strChainIdAndPos.substring(0, posInStr));
				}
			} catch (final NumberFormatException e) {
				// Log.log(Log.FATAL, "Message Reply: ChainIdAndPos is not a
				// valid number!", nfEx);
				// only warn
				Log.log(Log.WARN, "Message Reply: ChainIdAndPos is not a valid number!");
				return ErrorState.STAT_FALSE.getStateNumber();
			}
			...
			return replyMessageChain(null, chainID, chainPos, confirmCode, replyType, replyAdress);
			...
			try {
				...
				Log.log(Log.INFO, "DistributorWork.responseMessage(): + groupNum=" + groupNum + ", userNum=" + userNum);
				status = Short.parseShort(strStatus);
			} catch (final NumberFormatException e) {
				// Log.log(Log.FATAL, "Message Change Status: first three values
				// have to be numeric!");
				// only warn
				Log.log(Log.WARN, "Message Change Status: first three values have to be numeric!");
				return ErrorState.STAT_FALSE.getStateNumber();
			}
			...
			if (!(status == 0 || status == 1)) {
				Log.log(Log.WARN, "unknown state: " + status + " for msg " + txt);
				final UserTObject userTmp = UserDAO.select(memoryCacheDb, userNum);
				if (userTmp != null && userTmp.getStatusCode().equalsIgnoreCase(statusCode))// NOT_OK
				// -
				// main
				// system
				// temporarly
				...
				// available
				{
					HistoryWriter.logHistoryChangeStatus(localAppDb, userTmp, "Unknown state.", null, txt, status, reason, replyType, replyAdress);
					sendChangeStatusConfirmation(userTmp, txt + MSGCODE_UNKNOWN_STATUS, replyType, replyAdress,
							TextType.STATUSCHANGE_NOK.getTextTypeNumber());
				} else {
					HistoryWriter.logHistoryChangeStatus(localAppDb, null, "Unknown state.", null, txt, status, reason, replyType, replyAdress);
				}
			...
			if (changeGroupState == true) {
				...
				try // try finally
				{
					for (int i = 0; i < 3; i++) {
						...
						try {
							groupOra = changeGroupState(oraDb, Arrays.asList(oraDb), groupNum, userNum, status, statusCode, reason, txt, replyType,
									replyAdress);
						} catch (final Exception e) {
							sleep(10000);
							AmsConnectionFactory.closeConnection(oraDb); // don't
						...
						{
							try {
								final UserTObject user1 = UserDAO.select(memoryCacheDb, userNum);
								final UserGroupTObject group2 = changeGroupState(memoryCacheDb, Arrays.asList(memoryCacheDb, localAppDb), groupNum,
										userNum, status, statusCode, reason, txt, replyType, replyAdress);
								if (group2 != null) // OK - all well done
								{
									final UserGroupTObject ug = UserGroupDAO.select(memoryCacheDb, groupNum);
									HistoryWriter.logHistoryChangeStatus(localAppDb, null, null, ug, txt, status, reason, replyType, replyAdress);
									sendChangeGroupStatusConfirmation(user1, ug, "Group state changed: " + txt + MSGCODE_OK, replyType, replyAdress,
							...
							HistoryWriter.logHistoryChangeStatus(localAppDb, null,
									"Critical error => can update local db => replicate configuration.", null, txt, status, reason, replyType,
									replyAdress);
							Log.log(Log.FATAL, "critical error => can update local db => replicate configuration " + txt);
						}
				...
				try // try finally
				{
					for (int i = 0; i < 3; i++) {
						...
						try {
							userOra = changeUserStatusInGroup(oraDb, Arrays.asList(oraDb), groupNum, userNum, status, statusCode, reason, txt,
									replyType, replyAdress);
							if (userOra != null) {
								try {
									changeUserStatusInGroupInSyncTable(oraDb, oraDb, groupNum, userNum, status, reason);
						...
						{
							try {
								final UserTObject user2 = changeUserStatusInGroup(localAppDb, Arrays.asList(memoryCacheDb, localAppDb), groupNum,
										userNum, status, statusCode, reason, txt, replyType, replyAdress);

								if (user2 != null) // OK - all well done
								{
									final UserGroupTObject ug = UserGroupDAO.select(memoryCacheDb, groupNum);
									HistoryWriter.logHistoryChangeStatus(localAppDb, user2, null, ug, txt, status, reason, replyType, replyAdress);
									sendChangeStatusConfirmation(user2, txt + MSGCODE_OK, replyType, replyAdress,
											TextType.STATUSCHANGE_OK.getTextTypeNumber());
									Log.log(Log.INFO, txt + MSGCODE_OK);
									return ErrorState.STAT_OK.getStateNumber(); // All
																				// O.K.
				...
				// database
				{
					Log.log(this, Log.FATAL, "Could not changeStatus: temporary no connection to configuration database for msg " + txt);
					final UserTObject userTmp = UserDAO.select(memoryCacheDb, userNum);
					HistoryWriter.logHistoryChangeStatus(localAppDb, userTmp, "No connection to config db.", null, txt, status, reason, replyType,
							replyAdress);
					if (userTmp != null) {
						// available
						sendChangeStatusConfirmation(userTmp, txt + MSGCODE_NO_MAIN_SYSTEM, replyType, replyAdress,
								TextType.STATUSCHANGE_NOK.getTextTypeNumber());
					}
				}
	 ...
	 * @throws Exception
	 */
	private boolean changeUserStatusInGroupInSyncTable(java.sql.Connection readConnection, java.sql.Connection writeConnection, int groupNum,
			int userNum, short status, String reason) throws Exception {
		...
		if (!success) {
			HistoryWriter.logHistoryChangeStatus(localAppDb, user, "Failed to update status in SYNC-Table.", ug, "", status, reason, null, null);
	...
	private UserTObject changeUserStatusInGroup(java.sql.Connection readConnection, List<java.sql.Connection> writeConnections, int groupNum,
			int userNum, short status, String statusCode, String reason, String txt, String replyType, String replyAdress) throws Exception {

		final UserTObject user = UserDAO.select(readConnection, userNum);
		if (user == null) {
			HistoryWriter.logHistoryChangeStatus(localAppDb, null, "UserID=" + userNum + " not found.", null, txt, status, reason, replyType,
					replyAdress);
			// do not send back to user if not registered
			Log.log(Log.FATAL, "User not found: " + userNum + " for msg " + txt);
			// no user to send to
			return null;
		}

		// consitent state, but do not publish status code
		if (!user.getStatusCode().equalsIgnoreCase(statusCode)) {
			HistoryWriter.logHistoryChangeStatus(localAppDb, user, "Status code does not match.", null, txt, status, reason, replyType, replyAdress);
			Log.log(Log.FATAL, "Status code does not match for msg " + txt);
			return null;
		}

		final UserGroupTObject ug = UserGroupDAO.select(readConnection, groupNum);
		if (ug == null) {
			HistoryWriter.logHistoryChangeStatus(localAppDb, user, "GroupID=" + groupNum + " not found.", null, txt, status, reason, replyType,
					replyAdress);
			Log.log(Log.WARN, "no group: " + groupNum + " for msg " + txt);

			// never coming twice here
			sendChangeStatusConfirmation(user, txt + MSGCODE_NO_GROUP, replyType, replyAdress, TextType.STATUSCHANGE_NOK.getTextTypeNumber());
			return null;
		}

		// if update failed try max 3 times
		for (int i = 0; i < 3; i++) {
			final UserGroupUserTObject ugu = UserGroupUserDAO.select(readConnection, groupNum, userNum);
			if (ugu == null) {
				HistoryWriter.logHistoryChangeStatus(localAppDb, user, "User not in group.", ug, txt, status, reason, replyType, replyAdress);
				Log.log(Log.WARN, "User not in group for msg " + txt);
				// never coming twice here, nok not in group
				sendChangeStatusConfirmation(user, txt + MSGCODE_NOT_IN_GROUP, replyType, replyAdress, TextType.STATUSCHANGE_NOK.getTextTypeNumber());
				return null;
			}
			if (ugu.getActive() == status) {
				Log.log(Log.WARN, "Status already set for msg, handle as ok " + txt);
				return user;
			}

			// This condition is checked if one tries to change his/her status
			// via SMS to inactive.
			// only check if want to set 0 - Inactive
			if (status == 0) {
				int iActiveCount = 0;
				final Iterator<?> iter = UserGroupUserDAO.selectList(readConnection, groupNum).iterator();
				while (iter.hasNext()) {
					if (((AggrUserGroupUserTObject) iter.next()).getUserGroupUser().getActive() == 1) {
						// 0 - Inactive, 1 - Active (group ownership)
						// count active user in group
						iActiveCount++;
					}
				}

				if (ug.getMinGroupMember() >= iActiveCount) {
					HistoryWriter.logHistoryChangeStatus(localAppDb, user, "Min user count reached min=" + ug.getMinGroupMember() + ".", ug, txt,
							status, reason, replyType, replyAdress);
					Log.log(Log.WARN, "Min user count reached min=" + ug.getMinGroupMember() + " for msg " + txt);

					// never coming twice here
					// NOT_OK - min count of user reached
					sendChangeStatusConfirmation(user, txt + MSGCODE_MIN_USER_REACHED, replyType, replyAdress,
							TextType.STATUSCHANGE_NOK.getTextTypeNumber());
					return null;
				}
			}
			...
			for (java.sql.Connection writeConnection : writeConnections) {
				updateSuccessful &= UserGroupUserDAO.update(writeConnection, ugu);
			}
		...
		HistoryWriter.logHistoryChangeStatus(localAppDb, user, "Failed to update status (tried 3 times, Data changed or deleted!).", ug, txt, status,
				reason, replyType, replyAdress);
		Log.log(Log.WARN, "Failed to update status (tried 3 times) for msg " + txt + " (Data changed or deleted!)");
		return null;
	}

	private UserGroupTObject changeGroupState(final java.sql.Connection readConnection, List<java.sql.Connection> writeConnections,
			final int groupNum, final int userNum, final short status, final String statusCode, final String reason, final String txt,
		...
		if (groupUser == null) {
			HistoryWriter.logHistoryChangeStatus(localAppDb, null, "GroupID=" + groupNum + " or UserID=" + userNum + " not found.", null, txt,
					status, reason, replyType, replyAdress);
			Log.log(Log.FATAL, "User=" + userNum + " or group=" + groupNum + " not found");// do
																							// not
																							// send
																							// back
																							// to
		...
		final UserGroupTObject userGroup = UserGroupDAO.select(readConnection, groupNum);
		if (userGroup == null) {
			HistoryWriter.logHistoryChangeStatus(localAppDb, user, "GroupID=" + groupNum + " not found.", userGroup, txt, status, reason, replyType,
					replyAdress);
		...
		for (int i = 0; i < 3; i++) // if update failed try max 3 times
		{
			...
			if (ug == null) {
				...
				// never coming twice here, nok not in group
				sendChangeGroupStatusConfirmation(user, ug, txt + MSGCODE_NOT_IN_GROUP, replyType, replyAdress,
						TextType.STATUSCHANGE_NOK.getTextTypeNumber());
				return null;
			}
		...
		HistoryWriter.logHistoryChangeStatus(localAppDb, user, "failed to update group status (tried 3 times, Data changed or deleted!).", userGroup,
	...
	private void sendReplyConfirmationForUser(final UserTObject user, final boolean bOk, final String error, final int msgChainId,
			final int msgChainPos, final String replyType, final String originator) {
		String txt = null;
		String addr = "";
		try {
			int type = TextType.ALARMCONFIRM_NOK.getTextTypeNumber();
			if (bOk) {
				txt = "Alarm confirmation successful for MsgNo=" + prepareMessageNumber(msgChainId, msgChainPos);
				type = TextType.ALARMCONFIRM_OK.getTextTypeNumber();
			} else {
				txt = "Alarm confirmation rejected for MsgNo=" + prepareMessageNumber(msgChainId, msgChainPos) + " Error: " + error;
			}
			...
			if (replyType.equals(MSG_REPLY_TYPE_SMS)) {
				...
				Log.log(this, Log.INFO, "Reply Confirmation:'" + txt + "' send to user=" + user.getName() + " via sms (" + user.getMobilePhone()
						+ ")");
				...
				Log.log(this, Log.INFO, "Reply Confirmation:'" + txt + "' send to user=" + user.getName() + " via email(" + user.getEmail() + ")");
				...
				Log.log(this, Log.INFO, "Reply Confirmation:'" + txt + "' send to user=" + user.getName() + " via voicemail (" + user.getPhone()
						+ ")");
			}
		} catch (final Exception ex) {
			Log.log(this, Log.WARN, "failed to send reply confirmation message to user=" + user.getName() + addr, ex);
		}
	}
	 ...
	 */
	private void sendReplyConfirmationForJms(final TopicTObject topic, final int msgChainId, final int msgChainPos, final String originator) {
		String txt = null;
		String addr = "";
		try {
			txt = "Alarm confirmation successful for MsgNo=" + prepareMessageNumber(msgChainId, msgChainPos);
			...
			Log.log(this, Log.INFO, "Reply Confirmation:'" + txt + "' send to topic=" + topic.getTopicName() + " via JMS(" + topic.getTopicName()
					+ ")");
		} catch (final Exception ex) {
			Log.log(this, Log.WARN, "failed to send reply confirmation message to topic=" + topic.getTopicName() + addr, ex);
		}
	}

	private void sendChangeStatusConfirmation(final UserTObject user, String originalText, final String replyType, final String originator,
			final int texttype) {
		...
		try {
			...
			if (replyType.equals(MSG_REPLY_TYPE_SMS)) {
				...
				throw new AMSException("Invalid MSGPROP_REPLY_TYPE=" + replyType);
			}
		} catch (final Exception ex) {
			Log.log(this, Log.WARN, "failed to send change status confirmation message to user=" + user.getName() + addr, ex);
		}
	}

	private void sendChangeGroupStatusConfirmation(final UserTObject user, final UserGroupTObject group, String originalText, final String replyType,
		...
		try {
			...
			if (replyType.equals(MSG_REPLY_TYPE_SMS)) {
				...
				Vector<UserTObject> activeUsers = UserGroupUserDAO.selectByGroupAndState(memoryCacheDb, group.getID(), 1);
	...
	private int replyAllMessageChain(final String confirmCode, final String replyType, final String replyAdress) throws Exception {
		int iRet = ErrorState.STAT_OK.getStateNumber();

		final List<?> lMc = MessageChainDAO.selectKeyListByReceiverAdress(memoryCacheDb, MESSAGECHAIN_WORK, replyAdress);

		final Iterator<?> iter = lMc.iterator();
		while (iter.hasNext()) {
			final MessageChainTObject chainDb = (MessageChainTObject) iter.next();
			iRet = replyMessageChain(chainDb, chainDb.getMessageChainID(), chainDb.getReceiverPos(), confirmCode, replyType, replyAdress);
	...
	private int replyMessageChain(final MessageChainTObject chainDbParam, final int msgChainId, final int msgChainPos, final String confirmCode,
	...
	// JMSException
	{
		...
		if (chainDb != null && chainDb.getChainState() == MESSAGECHAIN_WORK) {
			if (chainDb.getReceiverPos() == msgChainPos) {
				fa = FilterActionDAO.select(memoryCacheDb, chainDb.getFilterActionRef());
				if (fa != null) {
					if (replyType.equals(MSG_REPLY_TYPE_JMS)) {
						final TopicTObject topic = TopicDAO.select(memoryCacheDb, fa.getReceiverRef());
						...
						HistoryWriter.logHistoryReply(localAppDb, "Reply", null, null, topic, "Chain replied for ChainId=" + msgChainId + ", Pos="
								+ msgChainPos + ".", fa.getFilterActionTypeRef(), msgChainId, msgChainPos, replyType, replyAdress);

						sendReplyConfirmationForJms(topic, msgChainId, msgChainPos, replyAdress);
						return ErrorState.STAT_OK.getStateNumber(); // All O.K.
					} else {
						userGroup = AggrUserGroupDAO.select(memoryCacheDb, fa.getReceiverRef(), msgChainPos);
						if (userGroup != null) {
							aUser = userGroup.getUsers().get(0);
						}

						String userConfirmCode = aUser.getUser().getConfirmCode();
						if (aUser != null && userConfirmCode != null && userConfirmCode.equals(confirmCode)) {
							chainDb.setChainState(MESSAGECHAIN_REPLIED);
							MessageChainDAO.update(localAppDb, chainDb);
							MessageChainDAO.update(memoryCacheDb, chainDb);

							HistoryWriter.logHistoryReply(localAppDb, "Reply", aUser.getUser(), userGroup.getUsergroup(), null,
									"Chain replied for ChainId=" + msgChainId + ", Pos=" + msgChainPos + ".", fa.getFilterActionTypeRef(),
									msgChainId, msgChainPos, replyType, replyAdress);

							sendReplyConfirmationForUser(aUser.getUser(), true, null, msgChainId, msgChainPos, replyType, replyAdress);
							return ErrorState.STAT_OK.getStateNumber(); // All
																		// O.K.
						}
					}
		...
		if (chainDb != null) {
			...
			if (fa != null && (userGroup == null || userGroup.getUsers().isEmpty())) {
				userGroup = AggrUserGroupDAO.select(memoryCacheDb, fa.getReceiverRef(), msgChainPos);
				if (userGroup != null) {
					aUser = userGroup.getUsers().get(0);
				}
			...
			if (aUser != null && fa != null) {
				...
				sendReplyConfirmationForUser(aUser.getUser(), false, err, msgChainId, msgChainPos, replyType, replyAdress);
			}
		} else {
			Log.log(Log.FATAL, "Message Reply: ChainID '" + msgChainId + "' not found.");
		}

		HistoryWriter.logHistoryReply(localAppDb, "Reply Err", aUser == null ? null : aUser.getUser(),
				userGroup == null ? null : userGroup.getUsergroup(), null, "Reply not accepted for ChainId=" + msgChainId + ", Pos=" + msgChainPos
						+ " Error: " + err, fa == null ? 0 : fa.getFilterActionTypeRef(), msgChainId, msgChainPos, replyType, replyAdress);
	...
	private int workOnMessageChain(final int msgChainId) throws Exception {
		...
		try {
			...
			if (msgChain.getNextActTime() != null) {
				if (System.currentTimeMillis() < msgChain.getNextActTime().getTime()) {
					return ErrorState.STAT_OK.getStateNumber(); // handle as
																// O.K. (no
					// error)
				}
			...
			try {
				...
				MessageDAO.select(memoryCacheDb, msgChain.getMessageRef(), extMsg);
			} catch (final JMSException e) // JMSException (STAT_ERR_JMSCON_EXT)
			{
				Log.log(this, Log.FATAL, "workOnMessageChain: could not extSession.createMapMessage", e);
				return ErrorState.STAT_ERR_JMSCON_EXT.getStateNumber();
			}

			final FilterTObject filter = FilterDAO.select(memoryCacheDb, msgChain.getFilterRef());
			final FilterActionTObject filterAction = FilterActionDAO.select(memoryCacheDb, msgChain.getFilterActionRef());
			...
			if (filterAction != null && filter != null) {
				userGroup = AggrUserGroupDAO.selectList(memoryCacheDb, filterAction.getReceiverRef()); // ggf.
																										// eine
																										// Topic-ID!
				...
				while (iter.hasNext()) {
					...
					if (aUser.getUserGroupUser().getActive() == 0) {
						Log.log(Log.WARN, "UserGroupRel not active: User " + aUser.getUser().getUserID() + " of Group "
								+ aUser.getUserGroupUser().getUserGroupRef() + " in FilterAction: " + filterAction.getFilterActionID());
						continue;
					}
					if (aUser.getUser().getActive() == 0) {
						Log.log(Log.WARN, "User not active: " + aUser.getUser().getUserID() + " in FilterAction: " + filterAction.getFilterActionID());
						continue;
					}
					...
					if (-1 == aUser.getUserGroupUser().getPos()) {
						// => config
						// Error
						throw new AMSException("Config Error: next UserPos == -1 == aUser.getUserGroupUser().getPos()");
					}

					if (msgChain.getReceiverPos() < aUser.getUserGroupUser().getPos())// act
																						// <
																						// next
					{
						aNextUser = aUser;

						msgChain.setReceiverPos(aNextUser.getUserGroupUser().getPos());
						final long currentTime = System.currentTimeMillis();
						msgChain.setSendTime(new Date(currentTime));
						msgChain.setNextActTime(new Date(currentTime + userGroup.getUsergroup().getTimeOutSec() * 1000));
			...
			{
				int iPref = 0;
				final String text = prepareMessageText(extMsg, filter, filterAction, msgChain); // throws
				// no
				// JMSException
				// (STAT_ERR_JMSCON_EXT)
				final String chainIdAndPos = prepareMessageNumber(msgChain.getMessageChainID(), msgChain.getReceiverPos());
				...
				case USERFILTERALARMTYPE_JMS:
					final TopicTObject topic = TopicDAO.select(memoryCacheDb, filterAction.getReceiverRef());
				...
				case USERFILTERALARMTYPE_VM:
					publishToConnectorVoiceMail(text, user.getPhone(), chainIdAndPos, TextType.ALARM_WCONFIRM.getTextTypeNumber()); // VoiceMail
				...
				default:
					...
					case FILTERACTIONTYPE_VM_GR:
						publishToConnectorVoiceMail(text, user.getPhone(), chainIdAndPos, TextType.ALARM_WCONFIRM.getTextTypeNumber(),
					...
					case FILTERACTIONTYPE_MAIL_GR:
						...
						break;
					default:
						throw new AMSException("Configuration is invalid. FilterActionType=" + filterAction.getFilterActionTypeRef());
					}
					break;
				}
				...
				try {
					HistoryWriter.logHistorySend(localAppDb, extMsg, text, filterAction.getFilterActionTypeRef(), user,
							userGroup != null ? userGroup.getUsergroup() : null, msgChain.getReceiverPos(), msgChain.getMessageChainID(), iPref,
							TopicDAO.select(memoryCacheDb, filterAction.getReceiverRef()));
				} catch (final JMSException e) // JMSException
				// (STAT_ERR_JMSCON_EXT)
				{
					Log.log(this, Log.FATAL, "workOnMessageChain: could not logHistorySend", e);
					return ErrorState.STAT_ERR_JMSCON_EXT.getStateNumber();
				}
			} else {
				String err = "Chain failed for ChainId=" + msgChain.getMessageChainID() + ", Pos=" + msgChain.getReceiverPos()
						+ " Reason: nobody replied the chain.";
				if (filterAction == null || filter == null) // log if error only
				{
					err = "Chain failed for ChainId=" + msgChain.getMessageChainID() + ", Pos=" + msgChain.getReceiverPos()
							+ " Error: system error - configuration changed.";
				} else if (bOneActive == false) {
					err = "Chain broken up for ChainId=" + msgChain.getMessageChainID() + ", Pos=" + msgChain.getReceiverPos()
				...
				HistoryWriter.logHistoryReply(localAppDb, "Failed", null, null, null, err,
						filterAction == null ? 0 : filterAction.getFilterActionTypeRef(), msgChain.getMessageChainID(), msgChain.getReceiverPos(),
						null, null);
				...
				if (bOneActive == true || filterAction == null || filter == null) {
					...
					// (STAT_ERR_JMSCON_EXT)
					{
						Log.log(this, Log.FATAL, "workOnMessageChain: could not send to extPublisherAlarm", e);
						return ErrorState.STAT_ERR_JMSCON_EXT.getStateNumber();
					}
				}
			...
			Log.log(this, Log.FATAL, "could not workOnMessageChain, set message chain to failed", e);
