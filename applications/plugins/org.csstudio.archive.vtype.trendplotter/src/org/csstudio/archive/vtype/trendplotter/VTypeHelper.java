...
public class VTypeHelper
{
                                           ...
                                           final Timestamp time)
    {
        if (value instanceof VNumber)
        {
            ...
            return new ArchiveVNumber(time, number.getAlarmSeverity(), number.getAlarmName(), number, number.getValue(),((ArchiveVNumber)value).getQuality());
        }
        ...
        if (value instanceof VNumberArray)
        {
            ...
            return new ArchiveVNumberArray(time, number.getAlarmSeverity(), number.getAlarmName(), number,((ArchiveVNumberArray)value).getQuality(), number.getData());
        }
        if (value instanceof VEnum)
        {
     ...
     *          or <code>null</code> if value is not handled
     */
    public static VType transform(final VType value)
                                        
    {    String q="";
         if(value instanceof ArchiveVType)
             q=((ArchiveVType)value).getQuality();
        final Timestamp time =getTimestamp(value);
        if (value instanceof VStatistics)
        { 
            final VStatistics number = (VStatistics) value;
            return new ArchiveVStatistics(time, number.getAlarmSeverity(), number.getAlarmName(), number,q, number.getAverage(),number.getMin(),number.getMax(),number.getStdDev(),number.getNSamples());
        }
        if (value instanceof VNumber)
        {  
            final VNumber number = (VNumber) value;
        ...
        { 
            ...
            return new ArchiveVString(time, string.getAlarmSeverity(), string.getAlarmName(),((ArchiveVString)value).getQuality(), string.getValue());
            else    return new ArchiveVString(time, string.getAlarmSeverity(), string.getAlarmName(),null, string.getValue());
        }
        if (value instanceof VNumberArray)
        { 
            final VNumberArray number = (VNumberArray) value;
