...
public class MaintenanceView extends AbstractSubView<ArticleMaintenance> {
    ...
    private class RepairSelectionListener extends SimpleSelectionListener {
        ...
        public void widgetSelected(SelectionEvent e) {
            if (wf.isSelected(P("repairGroup"))) {
                loadGroup();
                setRepairDestination(false, false, true);
            } else if (wf.isSelected(P("repairAccount"))) {
                loadUser();
                setRepairDestination(true, false, false);
            } else if (wf.isSelected(P("repairCompany"))) {
                loadCompany();
                setRepairDestination(false, true, false);
            } else {
                loadGroup();
                setRepairDestination(false, false, true);
            }
    ...
    private class EmailSelectionListener extends SimpleSelectionListener {
        @Override
        public void widgetSelected(SelectionEvent e) {
            wf.setText(P("sendEmailTo"), wf.getText(P("email")));
        }
    }

    private void setRepairDestination(final boolean account, final boolean company, final boolean group) {
        getEditorInput().get().processData(new Func1Void<ArticleMaintenance>() {
            @Override
            public void apply(ArticleMaintenance articleMaintenance) {
                articleMaintenance.setRepairAccount(account);
                articleMaintenance.setRepairCompany(company);
                articleMaintenance.setRepairGroup(group);
            }
        });
    }
    ...
    private void loadEmailUser() {
        wf.setInput(P("email"), logUserService.findAllAndIncludeEmptySelectionUseEmail());
    }

    private void loadEmailGroup() {
        wf.setInput(P("email"), logGroupService.findAllAndIncludeEmptySelectionUseEmail());
    }
    ...
    public Some<Composite> build(CrudController<Article> crudController, ArticleMaintenance articleMaintenance,
        ...
        wf.text(composite, "location").label("in Location:", "gapleft 10, w 80!").hint("wrap").build();
        ...
        final Combo comboRepairDone = wf.combo(repairComposite, "repair").data(new ArrayList<TextValue>())
                .hint("w 150!, ay top").build();
        ...
        final Combo comboEmail = wf.combo(emailComposite, "email").data(new ArrayList<TextValue>()).hint("w 150!")
                .build();

        comboEmail.addSelectionListener(new EmailSelectionListener());
        ...
        getEditorInput().get().processData(new Func1Void<ArticleMaintenance>() {
            ...
            public void apply(ArticleMaintenance articleMaintenance) {
                ...
                if (StringUtils.isNotEmpty(articleMaintenance.getBeiFirma())) {
                    ...
                    articleMaintenance.setRepair(articleMaintenance.getBeiFirma());
                    loadCompany();
                } else if (StringUtils.isNotEmpty(articleMaintenance.getBeiGruppe())) {
                    repairDoneByGroup.setSelection(true);
                    articleMaintenance.setRepair(articleMaintenance.getBeiGruppe());
                    loadGroup();
                } else if (StringUtils.isNotEmpty(articleMaintenance.getBeiAccount())) {
                    repairDoneByAccount.setSelection(true);
                    articleMaintenance.setRepair(articleMaintenance.getBeiAccount());
                    loadUser();
                } else {
                    repairDoneByGroup.setSelection(true);
                    articleMaintenance.setRepair(null);
                    loadGroup();
                }

                if (articleMaintenance.getRepair() != null) {
                    comboRepairDone.setText(articleMaintenance.getRepair());
                }
                ...
                articleMaintenance.setEmail(articleMaintenance.getSendEmailTo());

                if (articleMaintenance.getEmail() != null) {
                    comboEmail.setText(articleMaintenance.getEmail());
                }
