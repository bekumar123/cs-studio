package org.csstudio.dct.ui.editor.form;
...
import org.csstudio.dct.ui.editor.ParameterValuesClipboard;
import org.csstudio.dct.ui.editor.WidgetUtil;
import org.csstudio.dct.ui.editor.rowadapter.BeanPropertyTableRowAdapter;
import org.csstudio.dct.ui.editor.rowadapter.ParameterValueTableRowAdapter;
 ...
 * 
 */
public final class InstanceForm extends AbstractForm<IInstance> {
    ...
    protected void doCreateControl(ExpandBar bar, final CommandStack commandStack) {
        ...
        parameterValuesTable = WidgetUtil.create3ColumnTableWithDescription(composite, commandStack);
        parameterValuesTable.getViewer().getControl()
                .setLayoutData(LayoutUtil.createGridDataForHorizontalFillingCell(200));
        parameterValuesTable.getViewer().getTable().setHeaderVisible(true);
    ...
    protected void doSetInput(IInstance instance) {
        ...
        updateWidgetState(instance);
    }

    private void updateWidgetState(IInstance instance) {
        if ((instance != null) && (instance.getRootFolder() != null)) {
            boolean isInLibraryFolder = instance.getRootFolder().isLibraryFolder();
            copyButton.setEnabled(!isInLibraryFolder);
            pasteWithValuesButton.setEnabled(!isInLibraryFolder);
            parameterValuesTable.getViewer().getTable().setEnabled(!isInLibraryFolder);
