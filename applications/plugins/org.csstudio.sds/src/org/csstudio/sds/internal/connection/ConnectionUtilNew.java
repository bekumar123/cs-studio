...
public final class ConnectionUtilNew {
	...
	public static void connectToWidgetManagementApi(final AbstractWidgetModel widgetModel) {
		final String permissionId = widgetModel.getPermissionID();
		if ((permissionId != null) && (permissionId.length() > 0)) {
			ActivationService.getInstance().registerObject(permissionId, widgetModel);
		}
	}
	 ...
	 * 
	 */
	public static void connectDynamizedProperties(final WidgetProperty property, final Map<String, String> aliases,
			final boolean writeAccessAllowed, final IListenerRegistry registry, final ISimpleDalBroker broker) {
		...
		if (dynamicsDescriptor != null) {
				...
				if (ruleDescriptor != null) {
					...
					for (final ParameterDescriptor p : parameters) {
						...
						if (processVariable != null) {
							...
							// TODO 30.10.2010 (hrickens) workaround until Enum
							...
							final ChannelInputProcessor processor =
									new ChannelInputProcessor(
											p,
											ruleEngine,
											property,
											dynamicsDescriptor.getConnectionStateDependentPropertyValues(),
											dynamicsDescriptor.getConditionStateDependentPropertyValues()
									);

							final String characteristic = processVariable.getCharacteristic();

							final SinglePropertyReadConnector connector = new SinglePropertyReadConnector(processor, valueType,
									characteristic);
			...
			final ParameterDescriptor parameter = dynamicsDescriptor.getOutputChannel();

			if ((parameter != null) && writeAccessAllowed) {
				final IProcessVariableAddress processVariable = parameter.getPv(aliases);

				if (processVariable != null) {
					final ValueType type = determineValueType(property, processVariable);
					final SinglePropertyWriteConnector connector = new SinglePropertyWriteConnector(processVariable, type, broker);
	...
	public static void connectToBehavior(final AbstractWidgetModel widget, final IListenerRegistry registry) {
		final String behaviorId = widget.getBehaviorProperty(AbstractWidgetModel.PROP_BEHAVIOR);
		if ((behaviorId != null) && (behaviorId.length() > 0)) {
			final AbstractBehavior behavior = SdsPlugin.getDefault().getBehaviourService().getBehavior(behaviorId, widget.getTypeID());
			...
			if ((behavior != null) && (mainPv != null)) {
				...
				final ConnectionParameters connectionParameters = new ConnectionParameters(new RemoteInfo(RemoteInfo.DAL_TYPE_PREFIX
						+ "EPICS", mainPv.getProperty(), null, null), javaType);

				if (connectionParameters != null) {
					if (behavior != null) {
						final BehaviorConnector behaviorConnector = new BehaviorConnector(widget, connectionParameters, behavior);

						// .. let the behavior initialize the widget before any
						// connections are opened
						// hrickens (23.07.2010): move up to initializeWidget
						// before connect.
						// behavior.initializeWidget(widget);

						// .. connect to the control system to receive dynamic
						// values and meta data
						registry.register(connectionParameters, behaviorConnector);
						...
						for (final String id : settablePropertyIds) {
							widget.addPropertyChangeListener(id, behaviorConnector);
						}
					}
				}
			}
	 ...
	 */
	private static ValueType determineValueType(final WidgetProperty property, final IProcessVariableAddress processVariable) {
	...
	// TODO (jhatje): remove if patch in jca lib works
	// private static boolean isStringRecord(IProcessVariableAddress
	// processVariable) {
	...
	// }
	// }
	// ArrayList<String> recordTailsRegExp =
	// SdsPlugin.getDefault().getRecordTailsRegExp();
