...
class EnableDisableActionHelper {
    ...
    public static void changeMenuTextForRecord(IAction action, List<IElement> selection, String text) {
        checkNotNull(action);
        checkNotNull(selection);
        checkNotNull(text);
        
        if (isValidSelectionForDisable(selection) || isValidSelectionForEnable(selection)) {
            if (selection.size() == 1) {
                if (selection.get(0) instanceof IRecord) {
                    action.setText(text + " Record");
                    return;
                }
            }
        }
        action.setText(text + " all Records");
    }
