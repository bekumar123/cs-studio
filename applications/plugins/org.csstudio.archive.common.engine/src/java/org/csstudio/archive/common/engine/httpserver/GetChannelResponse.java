...
class GetChannelResponse extends AbstractChannelResponse {
    ...
    /**
     * @author wxu
     * channel table with Conn. state and Adel column
     */
    private void createChannelTable(@Nonnull final ArchiveChannelBuffer<?, ?> channel) {
        ...
        final ConnectionState state=channel.getConnectionState();
        ...
        final RingBuffer<IArchiveSample<?,?>> ringbuffer=channel.getLastSamples();
        for(int i=0; i<ringbuffer.size();i++){
            @SuppressWarnings("rawtypes")
            ISystemVariable variable = ringbuffer.get(i).getSystemVariable();
            tableLine(new String[]{ variable.getTimestamp().formatted(DATE_TIME_FORMAT), variable.getData().toString()});
        }
