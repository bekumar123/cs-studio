...
public class Status extends gov.aps.jca.ValuedEnum {
  ...
  static {
      ...
      int maxValue = 0;
      for(final java.util.Iterator it= _map.values().iterator(); it.hasNext(); ) {
          final Status t= (Status) it.next();
  ...
  protected Status(final String name, final int value) {
    super(name,value, _map);
  }

  static public Status forName(final String name) {
