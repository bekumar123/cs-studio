...
public class HibernateRepository implements IRepository {
	 ...
	 * @since 24.01.2012
	 */
	private final class LoadChannelHibernateCallback implements
			IHibernateCallback {
		...
		public ChannelDBO execute(@Nonnull final Session session) {
			...
			if (true) {
				...
				/*
				 * TODO (hrickens)[25.01.2012]: Das ist der gewünschte Weg.
				 * Funktioniert leider nicht da Hibernate mit der Momentanen
				 * konfiguration mit den Genarics nicht zurecht kommt. Es gibt
				 * vieleicht eine Möglichkeit die Funktioniert die aber eine
				 * Konfiguration über XML und Subclasses ntig macht. (siehe z.b.
				 * Buch: Hibernate Das Praxisbuch für Entwickler /s.260
				 * Mappping-Beispiele)
				 */
		...
		private ChannelDBO useTreeWalkWay(@Nonnull final Session session) {
			...
			if (ids.size() == 7) {
				...
				final Query createQuery = session.createQuery(String.format(
						"select f from %s f where f.id = '%d'",
						FacilityDBO.class.getName(), remove.intValue()));
				try {
					nodeImpl = (AbstractNodeSharedImpl<?, ?>) createQuery
							.uniqueResult();
					channel = climbeTreeDown2Channel(nodeImpl, ids, _ioName);
				} catch (final ObjectNotFoundException e) {
					LOG.debug("Query: Object not found = "
			...
			if (uniqueResult instanceof BigDecimal) {
				...
				while (parentId != null) {
					createSQLQuery = session
							.createSQLQuery(String
									.format("select n.parent_id from DDB_NODE n where n.id = '%d'",
											parentId.intValue()));
	 ...
	 * @since 08.04.2010
	 */
	private static final class EpicsAddressHibernateCallback implements
			IHibernateCallback {
		private final String _ioName;
	...
	public HibernateRepository(
			@CheckForNull final IHibernateManager hibernateManager) {
	...
	@CheckForNull
	private ChannelDBO getChannelWithIOName(
			@Nonnull final Set<ChannelDBO> nodes,
			@CheckForNull final String ioName) {
		ChannelDBO channel = null;
		if (ioName != null) {
			for (final ChannelDBO node : nodes) {
				if (ioName.equals(node.getIoName())) {
					channel = node;
				}
			}
		}
	...
	@Override
	@Nonnull
	public final String getEpicsAddressString(@Nonnull final String ioName)
			throws PersistenceException {
		final IHibernateCallback hibernateCallback = new EpicsAddressHibernateCallback(
				ioName);
		final String epicsAddressString = _instance
	...
	public final List<String> getIoNames() throws PersistenceException {
		final IHibernateCallback hibernateCallback = new IHibernateCallback() {
			...
			public List<String> execute(@Nonnull final Session session) {
				final Query query = session
						.createQuery("select channel.ioName from "
								+ ChannelDBO.class.getName() + " as channel");
				final List<String> ioNames = query.list();
				return ioNames;
			}
		};
		final List<String> ioNameList = _instance
				.doInDevDBHibernateEager(hibernateCallback);
	...
	@Override
	@Nonnull
	public final List<String> getIoNames(@Nonnull final String iocName)
			throws PersistenceException {
		final IHibernateCallback hibernateCallback = new IHibernateCallback() {
			@Override
			@SuppressWarnings("unchecked")
		...
		List<String> doInDevDBHibernateEager = _instance
				.doInDevDBHibernateEager(hibernateCallback);
	...
	@CheckForNull
	@Nonnull
	public final List<Integer> getRootPath(final int id)
			throws PersistenceException {
		final IHibernateCallback hibernateCallback = new IHibernateCallback() {
			@SuppressWarnings("unchecked")
			@Override
			...
			public List<Integer> execute(@Nonnull final Session session) {
				...
				while (searchId > 0) {
					...
					final BigDecimal uniqueResult = (BigDecimal) query
							.uniqueResult();
		...
		return doInDevDBHibernateLazy == null ? new ArrayList<Integer>()
				: doInDevDBHibernateLazy;
	}

	@Override
	@Nonnull
	public final String getShortChannelDesc(@Nonnull final String ioName)
			throws PersistenceException {
		final IHibernateCallback hibernateCallback = new IHibernateCallback() {
			@Override
			@SuppressWarnings("unchecked")
			...
			public String execute(@Nonnull final Session session) {
				final Query query = session
						.createQuery("select channel.description from "
								+ ChannelDBO.class.getName()
								+ " as channel where channel.ioName like ?");
				...
				return split[0].length() > 40 ? split[0].substring(0, 40)
						: split[0];
			}
		};
		final String doInDevDBHibernateEager = _instance
				.doInDevDBHibernateEager(hibernateCallback);
	...
	@Override
	@CheckForNull
	public final <T> List<T> load(@Nonnull final Class<T> clazz)
			throws PersistenceException {
		final IHibernateCallback hibernateCallback = new IHibernateCallback() {
			@Override
			@SuppressWarnings("unchecked")
	...
	@Override
	@CheckForNull
	public final <T> T load(@Nonnull final Class<T> clazz,
			@Nonnull final Serializable id) throws PersistenceException {
		final IHibernateCallback hibernateCallback = new IHibernateCallback() {
			@Override
			@SuppressWarnings("unchecked")
			@CheckForNull
			public T execute(@Nonnull final Session session) {
				final List<T> nodes = session.createQuery(
						"select c from " + clazz.getName() + " c where c.id = "
								+ id).list();
	...
	public final ChannelDBO loadChannel(@Nullable final String ioName)
			throws PersistenceException {
		final IHibernateCallback hibernateCallback = new LoadChannelHibernateCallback(
				ioName);
		return _instance.doInDevDBHibernateLazy(hibernateCallback);
	}
	...
	@Override
	@CheckForNull
	public final Channel4ServicesDBO loadChannelWithInternId(
			@Nullable final String internId) throws PersistenceException {
		final IHibernateCallback hibernateCallback = new IHibernateCallback() {
			@SuppressWarnings("unchecked")
			@Override
			@CheckForNull
			public Channel4ServicesDBO execute(@Nonnull final Session session) {
				...
				final Query createQuery = session.createQuery("select c from "
						+ Channel4ServicesDBO.class.getName()
						+ " c where c.krykNo = '" + internId + "'");
				final Channel4ServicesDBO nodes = (Channel4ServicesDBO) createQuery
						.uniqueResult();
				return nodes;
			}
		};
	...
	@Override
	@CheckForNull
	public final List<PV2IONameMatcherModelDBO> loadPV2IONameMatcher(
			@Nullable final Collection<String> ioName)
			throws PersistenceException {
		final IHibernateCallback hibernateCallback = new IHibernateCallback() {
			@SuppressWarnings("unchecked")
			@Override
			@CheckForNull
			public List<PV2IONameMatcherModelDBO> execute(
					@Nonnull final Session session) {
				if (ioName == null || ioName.isEmpty()) {
					return null;
				}
				final StringBuilder statement = new StringBuilder(
						"select pv from ").append(
						PV2IONameMatcherModelDBO.class.getName()).append(
						" pv where ");
				...
				for (final String string : ioName) {
					...
					statement.append(String.format("pv.epicsName = '%s'",
							string));
					notFirst = true;
				}
				final Query createQuery = session.createQuery(statement
						.toString());
	...
	@Override
	@CheckForNull
	public final PV2IONameMatcherModelDBO loadIOName2PVMatcher(
			@Nullable final String ioName) throws PersistenceException {
		final IHibernateCallback hibernateCallback = new IHibernateCallback() {
			@SuppressWarnings("unchecked")
			@Override
			@CheckForNull
			public PV2IONameMatcherModelDBO execute(
					@Nonnull final Session session) {
				if (ioName == null || ioName.isEmpty()) {
					return null;
				}
				...
				return (PV2IONameMatcherModelDBO) session.createQuery(
						statement.toString()).uniqueResult();
			}
		};
		return _instance.doInDevDBHibernateEager(hibernateCallback);
	}
	...
	@Override
	@CheckForNull
	public final SensorsDBO loadSensor(@Nonnull final String ioName,
			@Nonnull final String selection) throws PersistenceException {
		final IHibernateCallback hibernateCallback = new IHibernateCallback() {
			@Override
			@SuppressWarnings("unchecked")
			@CheckForNull
			public SensorsDBO execute(@Nonnull final Session session) {
				final String statment = "select" + " s" + " from "
						+ SensorsDBO.class.getName() + " s" + ", "
				...
				return sensors == null || sensors.size() < 1 ? null : sensors
						.get(0);
			}
		};
		return _instance.doInDevDBHibernateEager(hibernateCallback);
	...
	@Override
	@Nonnull
	public final List<SensorsDBO> loadSensors(@Nonnull final String ioName)
			throws PersistenceException {
		final IHibernateCallback hibernateCallback = new IHibernateCallback() {
			@Override
			@SuppressWarnings("unchecked")
	 ...
	 */
	@Override
	public final void removeGSDFiles(@Nonnull final GSDFileDBO gsdFile)
			throws PersistenceException {
	 ...
	 */
	@Override
	public final <T extends DBClass> void removeNode(@Nonnull final T dbClass)
			throws PersistenceException {
		_instance.doInDevDBHibernateLazy(new IHibernateCallback() {
	...
	@Override
	@Nonnull
	public final <T extends DBClass> T saveOrUpdate(@Nonnull final T dbClass)
			throws PersistenceException {
		try {
			_instance.doInDevDBHibernateLazy(new IHibernateCallback() {
	 ...
	 */
	@Nonnull
	public final GSDModuleDBO saveWithChildren(
			@Nonnull final GSDModuleDBO gsdModule) throws PersistenceException {
		try {
			_instance.doInDevDBHibernateLazy(new IHibernateCallback() {
				...
				private void saveChildren(
						@Nonnull final Set<ModuleChannelPrototypeDBO> moduleChannelPrototypes) {
