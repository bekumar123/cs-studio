...
public class ArticleSubViewDataProvider {
        ...
        return new GetOrCreate<ArticleInStore>().getObject(article, ArticleInStore.class,
                new FindEntity<ArticleInStore>() {
                    ...
                    public List<ArticleInStore> find(BigDecimal articleId) {
                        return articleService.findArticleInStore(articleId).getResponse();
                    }
                }, newStatus, oldStatus);
    }
    ...
    private Option<LagerArtikel> artikelAusDemLager(BindingEntity bindingEntity) {
        if (bindingEntity.isNew()) {
            try {
                ...
                ClearedPersistenceContextResponse<Option<ArticleInStore>> articleInStore = articleService
                        .findNewestEntryInStore(artikelDatenId);
                if (articleInStore.getResponse().hasValue()) {
                    Option<LagerArtikel> lagerArtikel = lagerArticleService.findById(articleInStore.getResponse().get()
                            .getLagerArtikelId());
    ...
    public ClearedPersistenceContextResponse<List<LagerArtikel>> calculateStoreMovements() {
        ...
        return new ClearedPersistenceContextResponse<List<LagerArtikel>>(merges);
    }
    ...
    public void mergeEntities() {
        for (BindingEntity bindingEntity : assignments.values()) {
            ...
            if (bindingEntity instanceof ArticleMaintenance) {
                updateRepairAssignment((ArticleMaintenance) bindingEntity);
                updateEmailAssignment((ArticleMaintenance) bindingEntity);
            }
    ...
    private void updateRepairAssignment(ArticleMaintenance bindingEntity) {
        bindingEntity.setBeiFirma(null);
        bindingEntity.setBeiGruppe(null);
        bindingEntity.setBeiAccount(null);
        if (bindingEntity.isRepairCompany()) {
            bindingEntity.setBeiFirma(bindingEntity.getRepair());
        } else if (bindingEntity.isRepairGroup()) {
            bindingEntity.setBeiGruppe(bindingEntity.getRepair());
        } else if (bindingEntity.isRepairAccount()) {
            bindingEntity.setBeiAccount(bindingEntity.getRepair());
        }
    }

    private void updateEmailAssignment(ArticleMaintenance bindingEntity) {
        bindingEntity.setSendEmailTo(bindingEntity.getEmail());
    }
