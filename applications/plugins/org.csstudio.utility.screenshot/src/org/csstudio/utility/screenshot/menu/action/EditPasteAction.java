...
public class EditPasteAction extends Action  implements IMenuListener {
    ...
    public EditPasteAction(final ScreenshotWorker w) {
        worker = w;
    ...
    public void run() {
        ...
        if(image != null) {
            ...
            MessageDialog.openError(worker.getShell(),
                                    worker.getNameAndVersion() + " - EditPasteAction",
                                    "Sorry, not possible to paste an image from the clipboard.");
        }
    }
}
