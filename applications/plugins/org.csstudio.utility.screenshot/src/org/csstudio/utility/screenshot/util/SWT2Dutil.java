...
public class SWT2Dutil {
     ...
     */
    public static Rectangle transformRect(final AffineTransform af, final Rectangle src){
        final Rectangle dest= new Rectangle(0,0,0,0);
        final Rectangle newSrc = absRect(src);
        Point p1=new Point(newSrc.x,newSrc.y);
        p1=transformPoint(af,p1);
        dest.x=p1.x; dest.y=p1.y;
        dest.width=(int)(newSrc.width*af.getScaleX());
        dest.height=(int)(newSrc.height*af.getScaleY());
        return dest;
    }
     ...
     * @return rectangle after transform with positive width and height
     */
    public static Rectangle inverseTransformRect(final AffineTransform af, final Rectangle src){
        final Rectangle dest= new Rectangle(0,0,0,0);
        final Rectangle newSrc = absRect(src);
        Point p1=new Point(newSrc.x,newSrc.y);
