...
/**
 * Button to set logscale for all PVs on active plot editor.
 * 
 * @author jhatje
 *
 */
public class LogScaleButton extends Button {
    /** Listener to invoke on button presses */
    private PlotListener listener = null;

    /** Initialize */
    public LogScaleButton() {
        Label icon = new Label(Activator.getDefault().getImage("icons/LogScale.ico"));
        setContents(icon);
