...
public class ImageCreator {
    ...
    public ImageCreator(final Display display) {
        this.display = display;
    }

    public void captureImages(final ImageBundle imageBundle) {
...
//            }
    ...
    private void normalizeRectangle(final Rectangle displayBounds, final Rectangle target) {
    ...
    private Rectangle getPartControl(final boolean section, final boolean shell) {
            ...
            for(; partControl != null; partControl = partControl.getParent()) {
                ...
                if(!shell) {
                    ...
                    if(!isView && partControl instanceof Composite) {
                        final Composite parent = ((Composite)partControl).getParent();
                    ...
                    if(isView) {
                        ...
                        final Point origin = partControl.toDisplay(0, 0);
                        final Rectangle bounds = partControl.getBounds();
                        if(section) {
                            ...
                            for(Control sectionControl = partControl; sectionControl != null; sectionControl = parent) {
                                ...
                                for(int i = 0; i < children.length; i++) {
                                    ...
                                    if(child instanceof Sash) {
                                        ...
                                        final Point sashSize = ((Sash)child).getSize();
                                        final int sashWidth = 2 * Math.min(sashSize.x, sashSize.y) - 1;
                                        final Rectangle parentBounds = parent.getBounds();
                                        final Point parentOrigin = parent.toDisplay(0, 0);
                                        ...
                                        for(int j = i; j < children.length; j++) {
                                            ...
                                            final Control c2 = children[j];
                                            if(c2 instanceof Sash) {

                                                final Point loc = c2.toDisplay(0, 0);
                                                final Point size = c2.getSize();
                                                if(size.x < size.y) {
                                                    if(loc.x < origin.x && loc.x > left) {
                                                        left = loc.x;
                                                    }
                                                    final int r = loc.x + size.x;
