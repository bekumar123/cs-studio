...
public class Stop implements IManagementCommand {
    ...
    public CommandResult execute(CommandParameters parameters) {
        ...
        if(param == null) {
            return CommandResult.createFailureResult(CommandResultPrefix.getErrorPrefix(1)
        ...
        if(password.length() > 0) {
            if(param.compareTo(password) != 0) {
                return CommandResult.createFailureResult(CommandResultPrefix.getErrorPrefix(2)
                                                         + " Invalid password");
            }
        }
        ...
        return CommandResult.createMessageResult(CommandResultPrefix.getOkPrefix()
                                                 + " Message Minder is stopping now...");
    }
}
