...
public class Controller implements ArchiveFetchJobListener {
    ...
    private volatile List<ITimeperiodUpdateListener> timeperiodUpdateListener = new ArrayList<ITimeperiodUpdateListener>();
    
    /** Listeners that are interested in the current chosen time index */
    private volatile List<ITimeChangeListener> timeChangeListener = new ArrayList<ITimeChangeListener>();
    ...
    public Controller(final Shell shell, final Model model, final Plot plot) {
        ...
        plot.addListener(new PlotListener() {
            ...
            public void syncTimeperiodWithHistoryControl() {
                DateTime startTime = new DateTime(model.getStartTime().toCalendar().getTimeInMillis());
...
//                System.err.println("ISO " + ISODateTimeFormat.basicTime().print(timeIndex));
    ...
    private void createAnnotations() {
        ...
        for (AnnotationInfo info : annotations) {
            ...
            annotation.setValues(info.getTimestamp().toDouble() * 1000.0, info.getValue());
    ...
    @Override
    public void archiveFetchFailed(final ArchiveFetchJob job, final ArchiveDataSource archive, final Exception error) {
        final String message = NLS.bind(Messages.ArchiveAccessMessageFmt, job.getPVItem().getDisplayName());
