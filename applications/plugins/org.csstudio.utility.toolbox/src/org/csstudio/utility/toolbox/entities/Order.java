...
public class Order extends BindingEntity implements TextValue, Cloneable<Order> {
   ...
   @Column(name = "ba_type_id")
   private BigDecimal baTypeId;
   ...
   // only used to store the selection from the combobox. baTypeId stores the real value.
   public String getBaType() {
      return baType;
   }

   public void setBaType(String baType) {
      pcs.firePropertyChange("baType", this.baType, baType);
      this.baType = baType;
   }

   public BigDecimal getBaTypeId() {
      return baTypeId;
   }

   public void setBaTypeId(BigDecimal baTypeId) {
      pcs.firePropertyChange("baTypeId", this.baTypeId, baTypeId);
