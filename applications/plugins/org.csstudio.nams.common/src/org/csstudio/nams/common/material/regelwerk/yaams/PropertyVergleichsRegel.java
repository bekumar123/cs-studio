		...
		try {
			switch (this.operator) {
			    ...
			    case OPERATOR_TEXT_EQUAL:
			        result = value.equalsIgnoreCase(vergleichsNachrichtValue);
			        break;
	            ...
	            case OPERATOR_NUMERIC_LT:
	                if (!value.isEmpty()) {
	                    result = this.numericCompare(value, vergleichsNachrichtValue) < 0;
	                } else {
	                    result = false;
	                }
	                break;
    			...
    			case OPERATOR_TIME_BEFORE:
    				result = this.timeCompare(value, vergleichsNachrichtValue) < 0;
    				break;
