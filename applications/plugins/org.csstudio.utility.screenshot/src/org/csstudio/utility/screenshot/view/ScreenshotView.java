...
public class ScreenshotView extends ViewPart implements FlavorListener {
    ...
    public static final String VIEW_ID = "screenshotView";
    ...
    public void createPartControl(final Composite parent) {
        ...
        final IMenuManager menuBar = actionBars.getMenuManager();
        ...
        // Create the menu
        final MenuManager fileMenu = new MenuManager(ScreenshotMessages.getString("ScreenshotView.MENU_FILE"));

        if(imageWorker != null) {
            final MenuManager fileSendMenu = new MenuManager(ScreenshotMessages.getString("ScreenshotView.MENU_FILE_SEND"));

            for (final IImageWorker element : imageWorker) {
                tempAction = new FileSendImageAction(worker, element);
                fileSendMenu.add(tempAction);

                if(element instanceof MailImageWorker) {
                    tempAction = new FileSendImageAction(worker, element);
        ...
        final MenuManager editMenu = new MenuManager(ScreenshotMessages.getString("ScreenshotView.MENU_EDIT"));
        final EditCopyAction copyAction = new EditCopyAction(worker);
        editMenu.add(copyAction);
        editMenu.addMenuListener(copyAction);

        final EditPasteAction pasteAction = new EditPasteAction(worker);
        editMenu.add(pasteAction);
        editMenu.addMenuListener(pasteAction);
        editMenu.add(new Separator());
        editMenu.add(new EditClearAction(worker));

        final MenuManager captureMenu = new MenuManager(ScreenshotMessages.getString("ScreenshotView.MENU_SELECTION"));
        ...
        final MenuManager restartMenu = new MenuManager(ScreenshotMessages.getString("ScreenshotView.MENU_RESTART"));
        ...
        final MenuManager helpMenu = new MenuManager(ScreenshotMessages.getString("ScreenshotView.MENU_HELP"));
